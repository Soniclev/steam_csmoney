...
from price_monitoring.parsers.steam.name_resolver.redis_cached_name_resolver import (
...
@pytest.fixture()
def name_resolver(root_resolver, fake_redis):
    return RedisCachedNameResolver(root_resolver, fake_redis)


async def test_resolve__not_cached(name_resolver, root_resolver, fake_redis):
...
async def test_resolve__cached(name_resolver, root_resolver, fake_redis):
    root_resolver.resolve_market_name.return_value = 123
...
async def test_resolve__not_existed_skin__not_cached(name_resolver, root_resolver, fake_redis):
...
async def test_resolve__not_existed_skin__cached(name_resolver, root_resolver, fake_redis):
    root_resolver.resolve_market_name.side_effect = SkinNotFoundError("AK")
