...
@pytest.fixture()
def parser(order_parser, skin_scheduler, steam_result_queue):
    return SteamOrderParser(order_parser, skin_scheduler, steam_result_queue)
...
async def test_run_steam_parser(
    parser, order_parser, steam_result_queue, skin_scheduler, market_name, is_success
):
    skin_scheduler.get_skin.return_value = market_name
    order_parser.fetch_orders.return_value = is_success

    await parser._run_steam_parser()

    skin_scheduler.get_skin.assert_called()
    if market_name:
        order_parser.fetch_orders.assert_awaited_with(
        ...
        order_parser.fetch_orders.assert_not_called()
        skin_scheduler.release_skin.assert_not_called()


async def test_run_steam_parser__with_error(
    parser, order_parser, steam_result_queue, skin_scheduler
):
    skin_scheduler.get_skin.return_value = "AK"
    order_parser.fetch_orders.side_effect = Exception()
